// Runs once, before any e2e worker starts. Makes the suite reproducible from a
// bare Postgres: create the test database if it is missing, put the schema in
// sync, and start from empty tables.
//
...
function pushSchema(): void {
  // prisma.config.ts reads the root `.env` through dotenvx, which does not
  // overwrite variables that are already set, so the DATABASE_URL handed to
  // the child process is the one Prisma ends up using.
  //
  // Deliberately no --accept-data-loss: on a database this suite created the
  // flag is never needed, and if someone's nafa_test has drifted we want a
